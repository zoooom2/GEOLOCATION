...
exports.checkLocation = async (data) => {
  ...
  if (insidePerimeters.length === 0) {
    console.log('not inside any territory');
    if (data.user.currentLocation) {
      // User is not inside any geofence, but has a current location
      // Change the user's current location to null and check them out
      // Remove their name from the list of users in the current location
      await User.findByIdAndUpdate(
        data.user._id,
        {
          currentLocation: null,
    ...
    const newLocationID = insidePerimeters[0]._id.toString();
    
    if (data.user.currentLocation !== newLocationID) {
      console.log('different current location from the perimeter own');
      if (data.user.currentLocation) {
        // User is inside a new geofence but has a different current location
        // Check them out from the current location
        await User.findByIdAndUpdate(
          data.user._id,
          {
      ...
      // Check the user into the new location
      await User.findByIdAndUpdate(data.user._id, {
        $push: {
          locationHistory: {
            location: newLocationID,
    ...
    } else if (!data.user.currentLocation) {
      console.log('first timer');
      // User is inside a new geofence and has not checked in before
      // Set the user's current location and update location history
      await User.findByIdAndUpdate(data.user._id, {
        currentLocation: newLocationID,
        $push: {
          locationHistory: {
